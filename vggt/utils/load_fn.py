...
def load_and_preprocess_images_square_from_numpy(image_list, target_size=1024):
    """
    Load and preprocess images by center padding to square and resizing to target size.
    Also returns the position information of original pixels after transformation.
    ...
    for img in image_list:
        ...
        # If there's an alpha channel, set RGB to black where alpha != 1
        if img.shape[-1] == 4:
            alpha = img[:, :, 3]
            mask = alpha == 0 
            img[mask, :3] = 0  # RGB置为黑色
            img = img[:, :, :3]  # 去掉alpha通道，后续处理为RGB
        ...
        # Create a new black square image and paste original
        square_img = Image.new("RGB", (max_dim, max_dim), (0, 0, 0))
        square_img.paste(Image.fromarray(img), (left, top))
