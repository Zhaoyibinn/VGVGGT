...
from vggt.models.depth_anything_3.cfg import create_object, load_config
from vggt.models.depth_anything_3.registry import MODEL_REGISTRY
from vggt.models.depth_anything_3.specs import Gaussians, Prediction
from vggt.models.depth_anything_3.utils.export import export
from vggt.models.depth_anything_3.utils.geometry import affine_inverse
from vggt.models.depth_anything_3.utils.io.input_processor import InputProcessor
from vggt.models.depth_anything_3.utils.io.output_processor import OutputProcessor
from vggt.models.depth_anything_3.utils.logger import logger
from vggt.models.depth_anything_3.utils.pose_align import align_poses_umeyama
from vggt.models.depth_anything_3.model.utils.transform import mat_to_quat, quat_to_mat, standardize_quaternion
...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def inference(
        ...
        with torch.autocast(device_type="cuda" if torch.cuda.is_available() else "cpu", enabled=False):
            prediction = self._align_to_input_extrinsics_intrinsics(
                extrinsics, intrinsics, prediction, align_to_input_ext_scale,ex_t_norm = ex_t_norm
            )
    ...
    def _transform_gaussians_sim3(self,
        gaussians: Gaussians | None, scale: float, rot: np.ndarray, trans: np.ndarray
    ) -> Gaussians | None:
        if gaussians is None:
            return None
        dtype = gaussians.means.dtype
        device = gaussians.means.device
        R_torch = torch.from_numpy(rot).to(dtype=dtype, device=device).float()
        t_torch = torch.from_numpy(trans).to(dtype=dtype, device=device).float()
        s_torch = torch.tensor(scale, dtype=dtype, device=device).float()

        means_flat = gaussians.means.reshape(-1, 3)
        transformed_means = s_torch * torch.matmul(means_flat, R_torch.t()) + t_torch
        means_new = transformed_means.reshape(gaussians.means.shape)

        scales_new = gaussians.scales * s_torch
        # R_ones = torch.eye(3, dtype=dtype, device=device)
        rotations_wxyz = gaussians.rotations
        rotations_xyzw = rotations_wxyz[..., [1, 2, 3, 0]]
        rot_mat = quat_to_mat(rotations_xyzw.reshape(-1, 4))
        rot_mat = rot_mat.reshape(gaussians.rotations.shape[:-1] + (3, 3))
        rot_mat_transformed = torch.matmul(R_torch, rot_mat)
        rotations_flat = mat_to_quat(rot_mat_transformed.reshape(-1, 3, 3))
        # rotations_xyzw = standardize_quaternion(rotations_flat.reshape(gaussians.rotations.shape))
        rotations_xyzw = rotations_flat.reshape(gaussians.rotations.shape)
        ...
        rot_mats_for_sh = R_torch.unsqueeze(0)
        harmonics_rotated = rotate_sh(harmonics_flat.unsqueeze(0), rot_mats_for_sh.unsqueeze(0).unsqueeze(0))
        harmonics_new = harmonics_rotated.reshape(harmonics_shape) 
    ...
    def _align_to_input_extrinsics_intrinsics(
        ...
        rot_norm, trans_norm, scale_norm, aligned_extrinsics_norm = align_poses_umeyama(
            extrinsics.numpy(),
            ex_t_norm.cpu().detach().float().numpy()[0],
        ...
        if prediction.gaussians is not None:
            prediction.gaussians = self._transform_gaussians_sim3(
                prediction.gaussians, scale_norm, rot_norm, trans_norm
            )
            # prediction.gaussians = self._transform_gaussians_sim3(
            #     prediction.gaussians, scale_1, rot_1, trans_1
            # )
    ...
    def _convert_to_prediction(self, raw_output: dict[str, torch.Tensor]) -> Prediction:
        ...
        if output.gaussians is not None:
            output.gaussians = self._cast_gaussians_float32(output.gaussians)
        end_time = time.time()
        logger.info(f"Conversion to Prediction Done. Time: {end_time - start_time} seconds")
        return output

    def _cast_gaussians_float32(self, gaussians: Gaussians) -> Gaussians:
        """Ensure gaussians tensors are float32 for downstream consumers."""
        return Gaussians(
            means=gaussians.means.to(torch.float32),
            scales=gaussians.scales.to(torch.float32),
            rotations=gaussians.rotations.to(torch.float32),
            harmonics=gaussians.harmonics.to(torch.float32),
            opacities=gaussians.opacities.to(torch.float32),
        )
