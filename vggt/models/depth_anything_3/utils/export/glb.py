...
def export_to_glb(
    ...
    _save_pointcloud_with_open3d(points, colors, export_dir)
...
def _save_pointcloud_with_open3d(
    points: np.ndarray, colors: np.ndarray, export_dir: str, filename: str = "scene_open3d.ply"
) -> str | None:
    if o3d is None:
        logger.warning("Open3D not installed; skipping ply export")
        return None
    if points.shape[0] == 0:
        logger.info("No points to export with Open3D")
        return None
    ...
    if colors_float.max() > 1.0:
        colors_float = np.clip(colors_float / 255.0, 0.0, 1.0)
    pc.colors = o3d.utility.Vector3dVector(colors_float)
    ply_path = os.path.join(export_dir, filename)
    o3d.io.write_point_cloud(ply_path, pc)
    logger.info(f"Saved Open3D point cloud: {ply_path}")
    return ply_path
