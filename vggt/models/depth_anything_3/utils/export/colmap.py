...
def export_to_colmap(
    ...
    logger.info(f"原始点云个数： {len(points)}")

    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points)
    pcd.colors = o3d.utility.Vector3dVector(colors / 255.0)

    downsample_voxel = 0.005
    down_pcd, voxel_indices, _ = pcd.voxel_down_sample_and_trace(
    ...
    selected_xyf = []
    for idx_list in voxel_indices:
        # if not idx_list:
        #     continue
        selected_xyf.append(points_xyf[idx_list[0]])
    # if selected_xyf:
    points_xyf = np.stack(selected_xyf, axis=0)
    # else:
    #     points_xyf = np.zeros((0, 3), dtype=np.int32)

    points = np.asarray(down_pcd.points)
    colors = np.asarray(down_pcd.colors) * 255.0
