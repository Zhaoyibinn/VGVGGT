...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        # assert len(images.shape) != 0, f"Images shape invalid for sequence {seq_name}"
        ...
        if "camera_valid_mask" in batch:
            sample["camera_valid_mask"] = torch.as_tensor(batch["camera_valid_mask"], dtype=torch.bool)
        if "has_depth" in batch:
            sample["has_depth"] = torch.as_tensor(batch["has_depth"], dtype=torch.bool)
        ...
        if batch['sdf'] is not None:
            tsdf = torch.from_numpy(batch['sdf']["tsdf"])
            voxel_xyz = torch.from_numpy(batch['sdf']["voxels"])
            voxel_weight = torch.from_numpy(batch['sdf']["weight"])
            sample['tsdf'] = tsdf
            sample['voxel_xyz'] = voxel_xyz
            sample['voxel_weight'] = voxel_weight
        return sample
