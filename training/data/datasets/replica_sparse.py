...
class ReplicaSparse(BaseDataset):
	def __init__(
		self,
		common_conf,
		split: str = "train",
		REPLICA_SPARSE_DIR: str = "train_data/replica_sparse",
		...
		self.REPLICA_SPARSE_DIR = REPLICA_SPARSE_DIR
		...
		logging.info(f"REPLICA_SPARSE_DIR is {self.REPLICA_SPARSE_DIR}")
		...
		if self.sequence_list_len == 0:
			raise ValueError(f"No valid ReplicaSparse scenes found in {self.REPLICA_SPARSE_DIR}")

		self._validate_scene_image_counts()

		status = "Training" if self.training else "Testing"
		logging.info(f"{status}: ReplicaSparse Data size: {self.sequence_list_len}")
		logging.info(f"{status}: ReplicaSparse dataset length: {len(self)}")

	def _build_scene_infos(self):
		scene_infos = []
		for scene_name in sorted(os.listdir(self.REPLICA_SPARSE_DIR)):
			scene_path = osp.join(self.REPLICA_SPARSE_DIR, scene_name)
			image_dir = osp.join(scene_path, "images")
			depth_dir = osp.join(scene_path, "depth_images")
			...
			if not frames:
				raise ValueError(f"No frames found in {images_path}")

			for frame in frames:
				frame["intrinsic"] = np.copy(camera_dict[frame["camera_id"]])
				stem = osp.splitext(frame["image_name"])[0]
				frame["image_path"] = osp.join(image_dir, frame["image_name"])
				frame["depth_path"] = osp.join(depth_dir, f"{stem}_depth.png")
				if not osp.isfile(frame["image_path"]):
					raise ValueError(f"Missing image file: {frame['image_path']}")
				if not osp.isfile(frame["depth_path"]):
					raise ValueError(f"Missing depth file: {frame['depth_path']}")
	...
	def _validate_scene_image_counts(self):
		...
		if len(unique_counts) > 1:
			raise ValueError(
				f"ReplicaSparse scene image counts are inconsistent: {unique_counts}. "
				"Please make every scene contain the same number of images before training."
			)
		...
		if self.fixed_scene_images:
			...
			if unique_counts[0] != self.expected_scene_image_count:
				raise ValueError(
					f"ReplicaSparse fixed-scene-images mode expects exactly {self.expected_scene_image_count} images "
					f"per scene, but found {unique_counts[0]}"
				)
	...
	def _load_colmap_cameras(self, camera_path: str):
		...
		with open(camera_path, "r", encoding="utf-8") as f:
			for line in f:
				...
				camera_dict[camera_id] = intrinsic
	...
	def _fixed_ids(self, seq_index: int, num_images: int, img_per_seq: int):
		...
		if self.fixed_triplet_ids is not None:
			...
			if len(ids) != img_per_seq:
				raise ValueError(
					f"fixed_triplet_ids length must equal img_per_seq={img_per_seq}, got {len(ids)}"
				)
	...
	def get_data(
		...
		if self.fixed_scene_images:
			if len(ids) != num_images or len(np.unique(ids)) != num_images:
				raise ValueError(
					f"ReplicaSparse fixed-scene-images mode requires all {num_images} unique images from scene {scene_basename}"
				)
			ids = np.sort(ids)
		...
		batch = {
			"seq_name": f"replica_sparse_{scene_basename}",
			"ids": ids,
