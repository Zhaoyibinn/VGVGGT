...
class TensorBoardLogger:
    ...
    Can either write only on rank 0 or create per-rank writers.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            log_all_ranks: Whether every distributed rank should create a writer
            rank_subdir: Whether to append a rank-specific subdirectory to path
        ...
        self._path: str = os.path.join(path, f"rank{self._rank}") if rank_subdir else path
        should_log = log_all_ranks or self._rank == 0

        if should_log:
            logging.info(
                f"TensorBoard SummaryWriter instantiated on rank {self._rank}. Files will be stored in: {self._path}"
            )
